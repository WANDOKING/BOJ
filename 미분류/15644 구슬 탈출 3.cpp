...
int main(void)
{
	...
	std::queue<std::pair<int, std::vector<char>>> q;
	q.push(std::make_pair(TO_HASH(0, redStartX, redStartY, blueStartX, blueStartY), std::vector<char>()));

	int result = -1;
	std::vector<char>* resultMoves = nullptr;
	while (q.empty() == false)
	{
		int moveCount = GET_MOVE_COUNT(q.front().first);
		int redX = GET_RED_X(q.front().first);
		int redY = GET_RED_Y(q.front().first);
		int blueX = GET_BLUE_X(q.front().first);
		int blueY = GET_BLUE_Y(q.front().first);
		std::vector<char>& moves = q.front().second;
		...
		{	
			q.push(std::make_pair(TO_HASH(moveCount + 1, redXAfterTilt, redYAfterTilt, blueXAfterTilt, blueYAfterTilt), moves));
		}
		moves.pop_back();
	...
	if (resultMoves != nullptr)
	{
		for (char move : *resultMoves)
		{
			std::cout << move;
		}
	}
