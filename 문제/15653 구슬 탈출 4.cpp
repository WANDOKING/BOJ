...
#define TO_HASH_WITHOUT_MOVE_COUNT(RX, RY, BX, BY) ((RX) * 1000 + (RY) * 100 + (BX) * 10 + BY)
...
int main(void)
{
	...
	visited.insert(TO_HASH_WITHOUT_MOVE_COUNT(redStartX, redStartY, blueStartX, blueStartY));
	...
	while (q.empty() == false)
	{
		...
		hash = TO_HASH_WITHOUT_MOVE_COUNT(redXAfterTilt, redYAfterTilt, blueXAfterTilt, blueYAfterTilt);
		if (gameResult == GAME_CLEAR)
		{
			result = moveCount + 1;
			break;
		}
		else if (gameResult == NONE && visited.find(hash) == visited.end())
		{
